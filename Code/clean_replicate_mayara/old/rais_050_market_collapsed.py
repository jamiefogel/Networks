...
def process_level2(
    ...
    df_for_theta = pd.merge(
        df[['ibgesubsector', 'emp', 'year']].drop_duplicates(),
    ...
    print('Checking shares')
    print(df_all.groupby(group_cols)['earndshare'].sum().value_counts())
    print(df_all.groupby(group_cols)['empshare'].sum().value_counts())

    # Keep relevant columns
    df_all[['cnpj_raiz','ibgesubsector', 'earndshare','earndshare2','empshare'] + market_vars].drop_duplicates()
    ...
    sum_trade = df_trade.groupby(group_key)['totdecearnmw'].sum().reset_index(name='sum_decearn')
    df_trade  = pd.merge(df_trade, sum_trade, on=group_key, how='inner')
    df_trade['Tearndshare'] = df_trade['totdecearnmw'] / df_trade['sum_decearn']

    df_trade = df_trade[['cnpj_raiz', 'Tearndshare','chng19941990TRAINS','chng19941990ErpTRAINS','chng19941990Kume'] + market_vars] 
    ...
    df_sums_merged = pd.merge(df_all, df_trade, on=merge_keys, how='inner')
    ...
    # 3f: ICE
    df_ice_1 = pd.merge(df_all, df_trade, on=merge_keys, how='inner')
    df_ice_2 = pd.merge(
        df_ice_1,
        df_beta[['cnpj_raiz'] + group_beta + ['betadw_rf','beta_rf']],
        on=['cnpj_raiz'] + group_beta,
        how='inner'
    )
    ...
    base_cols = ['cnpj_raiz', 'emp','ibgesubsector'] + market_vars
    ...
    merge_keys_flag = ['cnpj_raiz'] + market_vars
...
# These are the variables we actually need in the regression:
