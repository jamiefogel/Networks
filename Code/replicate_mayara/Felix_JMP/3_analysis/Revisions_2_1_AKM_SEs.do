	...
	foreach group in cbo942d none {
		...
		* Get ICE
		preserve
			...
			keep mmc `group' ice_dwtrains_hf
			gduplicates drop
			ren ice_dwtrains_hf 	iceH
			tempfile ice
			sa `ice'
		restore
		merge m:1 mmc `group' using `ice', keep(3) nogen
		
		/*
			merge m:1 cnae95 using `A_s2', keep(1 3) nogen
			/* Keep tradables in markets with any tradables */
			keep if !missing(A_s2) | iceH==.	
			/* Markets with no tradables have zero shock */
			replace iceH=0 if missing(iceH)	
			replace A_s2=0 if missing(A_s2)
		*/
		...
		* Sum firm Kappa weights by sector
		collapse (sum) Kappa_sm=Kappa_zm, by(cnae95 A_s2 mmc `group' iceH)

		keep cnae95 mmc `group' A_s2 Kappa_sm iceH
...
if `SEs'==1{
	foreach group in cbo942d none {
		...
		* Denominator : common across all markets
		u "${monopsonies}/dta/coeffs/`regsdate'/AKM_SE_firm_inputs_mmc_`group'.dta", clear
		keep mmc `group' iceH
		duplicates drop
		sum iceH, detail
		...
		u "${monopsonies}/dta/coeffs/`regsdate'/DD_stacked_annual_mktregs_reg_residuals_`group'.dta", clear
		des
		drop if year==1991
		tempfile fall
		sa `fall'
		...
		*/
		...
		outsheet using "${monopsonies}/csv/`regsdate'/AKM_SEs_mmc_`group'.csv", comma replace
	}
}
