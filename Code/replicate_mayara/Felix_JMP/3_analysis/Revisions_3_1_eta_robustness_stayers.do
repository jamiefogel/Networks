...
if `eta_regs'==1{
	...
	* Compute change in premia among strict stayers
	...
	u "${monopsonies}/sas/rais_lnearn_premia_firm_mmc_cbo942d_strict_stayers_`outdate'.dta", clear
	keep fakeid_firm mmc cbo942d year dprem_zro
	...
	* Store fixed effects for theta estimation in next step
	preserve
		...
		saveold "${monopsonies}/dta/coeffs/`outdate'/eta_change_delta_ro_strict_stayers.dta", replace
	restore
	drop delta_ro
	...
	outsheet using "${monopsonies}/csv/`outdate'/eta_change_regressions_strict_stayers.csv", comma replace
