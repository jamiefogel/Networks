...
capture log close 
log using "${encrypted}/logs/1_3_earnings_premia_mmc_cbo942d_firm.log", replace
...
if `premia'==1{
	...
	forvalues y=`yearfirst'(1)`yearlast'{		
		...
		* Use same categories as DK
		...
		gegen double fe_zro = group(fakeid_firm mmc cbo942d)
		...
		****************** MMC premia (excl. industry) ****************
		...
		keep fakeid_firm mmc cbo942d dprem_zro davgw_zro
		gduplicates drop
	...
	*********** Append across years *******
	...
	gduplicates drop
	compress
	saveold "${monopsonies}/sas/rais_lnearn_premia_firm_mmc_cbo942d_`outdate'.dta", replace
}
