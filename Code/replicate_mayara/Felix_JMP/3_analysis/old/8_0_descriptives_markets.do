...
if `firms'==1{
	...
	* Employment-weighted
	foreach stat in mean p10 p25 p50 p75 p90 {
		 ...
		 ds mmc cbo942d year, not
		 collapse (`stat') `r(varlist)' [w=mkt_emp], by(year)
 ...
 u `wmean', clear
 foreach stat in p10 p25 p50 p75 p90{
	merge 1:1 var year weight using `w`stat'', keep(3) nogen
 }
 
 append using `unw'
 
 outsheet using  "${monopsonies}/csv/`outdate'/market_descriptives_firms.csv", comma replace
