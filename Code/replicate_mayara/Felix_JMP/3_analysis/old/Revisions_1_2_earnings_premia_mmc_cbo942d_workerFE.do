...
if `premia'==1{
		...
		use fakeid_worker mmc cbo942d agegroup educ earningsdecmw using "${monopsonies}/sas/rais_for_earnings_premia1986.dta", clear
		...
		forvalues i=1987/2000{
			append using "${monopsonies}/sas/rais_for_earnings_premia`i'.dta", keep(fakeid_worker mmc cbo942d agegroup educ earningsdecmw)
			drop if mmc==13007 | mmc==23014
			drop if inlist(cbo942d,31,22,37)
			keep if !missing(educ) & earningsdecmw>0
		...
		*Market-year group
		gegen double fe_rot = group(mmc cbo942d year)
		...
		****************** Premia ****************
		reghdfe lndecearn, absorb(dprem_rot=fe_rot fe_type_year)
		

		keep if !missing(dprem_rot) 
		keep year mmc cbo942d dprem_rot
		gduplicates drop

		compress
	
	saveold "${monopsonies}/sas/rais_lnearn_premia_mmc_cbo942d_workerFE`outdate'.dta", replace
}
