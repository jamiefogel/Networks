...
if `premia'==1{
		...
		use mmc cbo942d agegroup educ earningsdecmw using "${monopsonies}/sas/rais_for_earnings_premia1986.dta", clear
		...
		*Market-year group
		gegen double fe_rt = group(mmc year)
		...
		****************** Premia ****************
		reghdfe lndecearn, absorb(dprem_rt=fe_rt fe_type_year)
		

		keep if !missing(dprem_rt) 
		keep year mmc dprem_rt
		gduplicates drop

		compress
	
	saveold "${monopsonies}/sas/rais_lnearn_premia_mmc_workerFE`outdate'.dta", replace
}
