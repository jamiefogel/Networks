/*
	Add scatterplot : concentration vs wages at baseline
...
foreach group in none cbo942d{
	...
	gen double avgw = ln(mkt_wdbill/mkt_emp)
	gen double lnemp = ln(mkt_emp)

	twoway scatter avgw hf_wdbill if year==1991 [w=mkt_emp], msymbol(Oh) msize(tiny) ///
		scheme(s1mono) ylabel(, angle(0)) ///
		ytitle("") subtitle("1991 market log average wage", place(w) size(small)) ///
		xtitle("1991 labor market concentration (Payroll Herfindahl)") 
	graph export "${monopsonies}/eps/`outdate'/descriptives_hhi_wage_mmc_`group'.eps", replace

	twoway scatter lnemp hf_wdbill if year==1991 [w=mkt_emp], msymbol(Oh) msize(tiny) ///
		scheme(s1mono) ylabel(, angle(0))  ///
		ytitle("") subtitle("1991 market log total employment", place(w) size(small)) ///
		xtitle("1991 labor market concentration (Payroll Herfindahl)") 
	graph export "${monopsonies}/eps/`outdate'/descriptives_hhi_emp_mmc_`group'.eps", replace
