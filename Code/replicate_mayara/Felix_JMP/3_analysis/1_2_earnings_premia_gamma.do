...
if `balanced_sample'==1{
	...
	forvalues y=`yearfirst'(1)`yearlast'{
		u gamma using "${monopsonies}/sas/rais_for_earnings_premia`y'_gamma.dta", clear
		gduplicates drop
	...
	u `gamma`yearfirst'', clear
	forvalues y=`yearfirst'(1)`yearlast'{
		merge 1:1 gamma using `gamma`y'', keep(3) nogen
	}
...
if `premia'==1{
	forvalues y=`yearfirst'(1)`yearlast'{
		...
		****************** MMC premia (excl. industry) ****************
		...
		keep gamma dprems_ro dprem_ro davgw_ro 
		gduplicates drop
	...
	*********** Append across years *******
	...
	gduplicates drop
	compress
	saveold "${monopsonies}/sas/rais_lnearn_premia_gamma_`outdate'.dta", replace
