...
if `setup'==1{
  ...
  foreach sampfile in 1 0 {
		...
		* XX ice_dwerp ice_dwtrains    these variables don't exist in the inputdata set. I think it should be the ones renamed below, but that's a guess
		...
		keep gamma ice_dwerp ice_dwtrains bemp
		gduplicates drop
		tempfile market
		sa `market'
		...
		* Bring in earnings premia
		...
		if `obsmktslist'==1{
			preserve
				...
				outsheet using "${monopsonies}/csv/`outdate'/etaregs_cbo942ds_3states.csv", comma replace
			restore
			...
			preserve
				...
				outsheet using "${monopsonies}/csv/`outdate'/etaregs_mmcs_3states.csv", comma replace
			restore
		}
		*/
		***********************************
		...
		* Drop certain mmcs
		// XX Probably want to actually keep these but I lost mmc somewhere upstream and don't want to figure that out right meow
		//drop if mmc==13007 
		...
		* Compute firm baseline share
		preserve
			 keep if year==`baseyear'
			 gegen double sumearn = sum(totmearn), by(gamma)
			 gen double bwshare = totmearn/sumearn
			 gegen double sumemp = sum(emp), by(gamma)
			 gen double beshare = emp/sumemp

			 gegen double Tsumearn = sum(totmearn) if T==1, by(gamma)
			 gen double bTwshare = totmearn/Tsumearn if T==1
			 gegen double Tsumemp = sum(emp) if T==1, by(gamma)
			 gen double bTeshare = emp/Tsumemp if T==1
			 
			 keep fakeid_firm gamma beshare bwshare bTeshare bTwshare 
			 tempfile shares
			 sa `shares'
		restore

		* Merge in base shares, replacing with zero if firm was not there
		merge m:1 fakeid_firm gamma using `shares', keep(1 3) nogen
		...
		* Flag Unique producers of each cnae95 within each market
		preserve
			...
			keep fakeid_firm cnae95 gamma
			bys cnae95 gamma: gegen producers = count(fakeid_firm)
			keep if producers==1
			keep fakeid_firm gamma
			
			gen up`baseyear_n' = 1
			tempfile unique 
			sa `unique'
		restore	
		merge m:1 fakeid_firm gamma using `unique', keep(1 3) nogen

		replace up`baseyear_n' = 0 if missing(up`baseyear_n')

		* Market that has unique producer 
		gegen up`baseyear_n'mkt_temp = max(up`baseyear_n'), by(gamma)
		...
		* Merge in market shocks
		merge m:1 gamma using `market', keep(3) nogen

		
		* Log employment
		gen double lnemp = ln(emp)
		
		gegen fe_zro = group(fakeid_firm gamma)
		...
		keep 	fakeid_firm gamma year cnae95 ///
				fe_zro bTwshare bTeshare  bwshare  beshare   ///
				chng* up`baseyear_n' up`baseyear_n'mkt T bemp // ice_dwerp ice_dwtrains Tnexplib bexp
		
		order	year fe_zro fakeid_firm gamma cnae95 up`baseyear_n' up`baseyear_n'mkt T bemp //Tnexplib explib  explib
				
		compress
		saveold "${monopsonies}/sas/eta_changes_regsfile`sampfile'_3states_gamma.dta", replace
...
if `eta_regs'==1{
	...
	u "${monopsonies}/sas/eta_changes_regsfile`usesample'_3states_gamma.dta", clear
	
	cap drop if inlist(cbo942d,31,22,37)
	
	* Cross-section FEs
	gegen fe_ro = group(gamma)
...
pause			
				preserve
					...
					keep fakeid_firm gamma `lhs' `rhs' delta_ro
	...
	** Append all main regression coefficients and export to csv ***
	...
	duplicates drop
	outsheet using "${monopsonies}/csv/`outdate'/eta_change_regressions_3states_gamma.csv", comma replace
	...
	******************************************************************************
	...
	duplicates drop
	compress
	saveold "${monopsonies}/dta/coeffs/`outdate'/eta_change_delta_ro_3states_gamma.dta", replace
