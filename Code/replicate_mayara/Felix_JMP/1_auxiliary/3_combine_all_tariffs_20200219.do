/*
	...
	20202019:	generate new graphs showing tariff variation
*/
...
if `newgraphs'==1{
	...
	u "${public}/Tariffs/cnae10_tariff_changes_`tariff_fy'_`tariff_ly'.dta", clear
	
	gen y = chng`tariff_ly'`tariff_fy'TRAINS*100
	
	* Change vs levels graph
	twoway scatter y TRAINS`tariff_fy', scheme(s1color) ///
	ytitle("Post-liberalization tariff change" "(% of pre- level)") ///
	xtitle("Pre-liberalization tariff level (%)") mcolor(blue)

	graph export "${monopsonies}/eps/`outdate'/TRAINS_`tariff_ly'`tariff_fy'change_vs_`tariff_fy'level.eps", replace
	
	drop y
	gen y = chng`tariff_ly'`tariff_fy'ErpTRAINS*100
	
	* Change vs levels graph
	twoway scatter y ErpTRAINS`tariff_fy', scheme(s1color) ///
	ytitle("Post-liberalization effective tariff change" "(% of pre- level)") ///
	xtitle("Pre-liberalization effective tariff level (%)") mcolor(blue)

	graph export "${monopsonies}/eps/`outdate'/ErpTRAINS_`tariff_ly'`tariff_fy'change_vs_`tariff_fy'level.eps", replace
