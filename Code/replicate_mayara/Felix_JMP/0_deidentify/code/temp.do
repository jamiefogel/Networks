...
if 1==1{
	...
	local files_dta `"`filtered_files'"'
	di `"`files_dta'"'
}
...
if `makefake_workers'==1{
	...
	* Testing
	if `testing'==1{
		...
		* XX cpf only exists starting in 2004 so If I only run throguh 2000  cpf won't exist which will cause this to fail
		* u workerid_pis workerid_cpf using "${IDlinksrais}/rais_workerIDs.dta", clear
		* The above line was the original; this is my new version
		capture u workerid_pis workerid_cpf using "${IDlinksrais}/rais_workerIDs.dta", clear
		if _rc==111{
			u workerid_pis using "${IDlinksrais}/rais_workerIDs.dta", clear
			gen double workerid_cpf = workerid_pis
		}
	}
...
if `makefake_estab'==1{
	...
	* XX We have one missing value out of 5 million. At some point could investigate why, but for now drop
	drop if mi(estabid_cnpjcei)
