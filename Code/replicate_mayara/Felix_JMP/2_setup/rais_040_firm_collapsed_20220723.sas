/*
	20220723
		a) Use updated cbo codes, drop government and military
			- monopsonies/dta/cbo94_compiled_flagged.dta
		b) Use updated ibgesubactivityXcnae95 map to decide obs to keep, which excludes government and military
			- monopsonies/dta/ibgesubactivityXcnae95_finalmap.dta
		c) 
...
%macro valid(i=);
	...
	%if &i<1994 %then %do;
		%let cboraw=cbo;
		%let cboclean=cbo94;
		%let validdata = cbo94_compiled_flagged;
	%end;
	%else %do;
		%let cboraw=cbo94;
		%let cboclean=cbo94;
	...
	/* 	Small number of firms have estabs with diff ibgesubsectors. 
		Assign the ibgesubsector for the largest establishment.
	*/	
		proc sql;
