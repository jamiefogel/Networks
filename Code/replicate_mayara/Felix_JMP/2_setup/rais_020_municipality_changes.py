...
for year in range(first_year, last_year):
    ...
    if not os.path.exists(f"{monopsas_path}/rais{year}.parquet") or not os.path.exists(f"{monopsas_path}/rais{next_year}.parquet"):
        print(f'File not found. Skipping {year}')
        continue
    rais_base = pd.read_parquet(f"{monopsas_path}/rais{year}.parquet", columns=["fakeid_estab", "municipality"])
    rais_next = pd.read_parquet(f"{monopsas_path}/rais{next_year}.parquet", columns=["fakeid_estab", "municipality"])
...
# Save to Parquet
location_changes.to_parquet(f"{monopsas_path}/rais_municipality_changes.parquet", index=False)
...
for year in range(first_year, last_year + 1):
    print(year)
    if not os.path.exists(f"{monopsas_path}/rais{year}.parquet"):
        print(f'File not found. Skipping {year}')
        continue
    # Load RAIS data for the year
    rais_year = pd.read_parquet(f"{monopsas_path}/rais{year}.parquet", columns=["fakeid_worker", "municipality"])
...
# Save to Parquet
municipality_counts.to_parquet(f"{monopsas_path}/rais_obs_by_municipality_year.parquet", index=False)
