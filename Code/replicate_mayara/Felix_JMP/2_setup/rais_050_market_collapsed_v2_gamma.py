...
def process_level2(level2, file_rais_collapsed, base_year=1991, export_path=EXPORT_PATH):
    ...
    # ----------------------------------------------------------------
    df_trad = pd.merge(
        df_base[['fakeid_firm', 'gamma', 'totdecearnmw','cnae95']],
    ...
    sum_trad = df_trad.groupby(['gamma'])['totdecearnmw'].sum().reset_index(name='sum_decearn')
    df_trad  = pd.merge(df_trad, sum_trad, on=['gamma'], how='inner')
    ...
    df_beta = df_beta.groupby(['gamma'], as_index=False).apply(compute_betas)

    # ----------------------------------------------------------------
    # 3e. "sums": sum of empshare, earndshare, earndshare2, Tearndshare
    # ----------------------------------------------------------------
    df_sums_merged = pd.merge(df_all, df_trad, on=['fakeid_firm', 'gamma'], how='inner')
    df_sums = df_sums_merged.groupby(['gamma'], as_index=False).agg({
        'empshare': 'sum',
        'earndshare': 'sum',
        'earndshare2': 'sum',
        'Tearndshare': 'sum'
    ...
    # ----------------------------------------------------------------
    df_ice_1 = pd.merge(df_all, df_trad, on=['fakeid_firm', 'gamma'], how='inner')
    df_ice_2 = pd.merge(
        df_ice_1,
        df_beta[['fakeid_firm', 'gamma','betadw_rf','beta_rf']],
        on=['fakeid_firm', 'gamma'], how='inner'
    )
    df_ice   = pd.merge(df_ice_2, df_sums, on=['gamma'], how='inner')
    ...
    df_ice_final = df_ice.groupby(['gamma'], as_index=False).apply(compute_ice)
    ...
    out_qtfile = f"{export_path}/regsfile_gamma_{base_year}_empT_pctiles.dta"
    ...
    df_tag = pd.merge(df_basey, df_q, on=['gamma'], how='left')
    # Also merge the "tradables" quantiles:
    df_tag = pd.merge(df_tag, df_qt, on=['gamma'], how='left', suffixes=('', '_trad'))
    ...
    df_shares_tags = pd.merge(df_shares, df_tag_for_merge, on=['fakeid_firm', 'gamma'], how='left')
    ...
    df_sizebins = df_shares_tags.groupby(['gamma', 'year'], as_index=False).apply(aggregator_flags)

    # Merge df_sizebins into df_mktout
    df_mktout = pd.merge(df_mktout, df_sizebins, on=['gamma', 'year'], how='left')

    # ----------------------------------------------------------------
    # 3l. Merge ICE shocks => final "regsfile" DataFrame
    # ----------------------------------------------------------------
    df_regsfile = pd.merge(df_mktout, df_ice_final, on=['gamma'], how='left')
