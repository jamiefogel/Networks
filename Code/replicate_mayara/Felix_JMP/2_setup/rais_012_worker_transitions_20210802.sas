...
proc sql;
	...
	select a.fakeid_worker,
		   ...
		   d.mmc 							as mmc_&y,
		   strip(put(d.mmc,5.) || "_" || put(coalesce(c.cbo942d,99),2.)) as mmc_cbo942d_&y,
		   strip(put(d.mmc,5.) || "_" || put(b.cnae952d_divisao,2.)) as mmc_cnae952d_&y,
		   e.cnae95  						as cnae95_&y	
	from monopsas.rais&y as a
		inner join monopsas.crosswalk_muni_to_mmc_DK17 as d
			on a.municipality=d.codemun  
		...
		inner join monopsas.&validdata as c
			on a.&cboraw  = c.&cboclean
	where (	c.cbo942d~=22	& c.cbo942d~=31 & c.cbo942d~=37 &
...
%macro moves(o=,d=);
	...
	%transmat(fromto=mmc_cbo942d);
	%transmat(fromto=mmc_cnae952d);
