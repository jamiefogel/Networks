...
# METHOD 2 FOR SPATIAL VARIANCE: Function to calculate Euclidean distance variance for a group
# DEFINITION: 
    # 1) calculates individual distances of each point to the centroid
    # 2) calculates the variance of these distances (related to the avg distance)
def euclidean_variance(group):
    mean_utm_lon = group['utm_lon'].mean()
    mean_utm_lat = group['utm_lat'].mean()
    
    distances = np.sqrt((group['utm_lon'] - mean_utm_lon) ** 2 + (group['utm_lat'] - mean_utm_lat) ** 2)
    var_distance = distances.var()
    
    return var_distance

# Group by 'iota' and calculate standard distance and Euclidean variance for each group
spatial_var = raw.groupby('iota').apply(lambda x: pd.Series({
    'std_distance': standard_distance(x),
    'euclidean_var': euclidean_variance(x)
})).reset_index()
>>>>>>> Bernardo: TODAY - 5/18/2024
...
# Function to calculate weighted Euclidean distance variance for a group
def weighted_euclidean_variance(df, weights):
    mean_utm_lon = np.average(df['utm_lon'], weights=weights)
    mean_utm_lat = np.average(df['utm_lat'], weights=weights)
    
    distances = np.sqrt((df['utm_lon'] - mean_utm_lon) ** 2 + (df['utm_lat'] - mean_utm_lat) ** 2)
    mean_distance = np.average(distances, weights=weights)
    weighted_var_distance = np.average((distances - mean_distance) ** 2, weights=weights)
    
    return weighted_var_distance
...
for iota, weights in weights_dict.items():
    ...
    var_distance = weighted_euclidean_variance(merged_df, merged_df['weight'])
    
    # Store results
    weighted_spatial_var.append({'iota': iota, 'weighted_std_distance': std_distance, 'weighted_euclidean_var': var_distance})
    count +=1
    print(str(count) + '/' + str(tot))
...
correlation_matrix = iotas_w_attributes[['avg_dist_centroid', 'auc_decay', 'std_distance', 'euclidean_var','educ_mean', 'educ_mean_rank', 'mean_monthly_earnings', 'mean_monthly_earnings_rank', 'num_unique_jids', 'num_unique_wids', 'num_unique_wid_jids']].corr()
#correlation_matrix = iotas_w_attributes[['avg_dist_centroid', 'auc_decay', 'std_distance', 'euclidean_var','weighted_std_distance', 'weighted_euclidean_var', 'educ_mean', 'educ_mean_rank', 'mean_monthly_earnings', 'mean_monthly_earnings_rank', 'num_unique_jids', 'num_unique_wids', 'num_unique_wid_jids']].corr()
>>>>>>> Bernardo: TODAY - 5/18/2024
