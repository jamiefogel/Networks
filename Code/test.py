...
mle_sums_data = est_mle_data_sums_filename
estimates_file = est_mle_estimates_filename
worker_type_var = wtype_var
job_type_var = jtype_var
...
term_earnings = -( mle_data['obs_employ'] * torch.log(sigma_par * torch.sqrt(2*torch.pi)) + mle_data['sum_logomega'] + (1/(2*torch.pow(sigma_par,2))) * (mle_data['sum_logomega_sq'] - 2*(torch.log(phi_par) * mle_data['sum_logomega_ig']).sum() + ( torch.pow(torch.log(phi_par),2) * mle_data['sum_count_ig'][:,1:]).sum()))
    return term_choice + term_earnings

torch.log(phi_par).shape
torch.pow(torch.log(phi_par),2).shape
