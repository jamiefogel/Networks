...
analysis_data.to_pickle('../dump/analysis_data_BM_temp.p')
#analysis_data = pd.read_pickle('../dump/analysis_data.p')

# Subset for Stata
vars = ['codemun', 'idade', 'genero', 'raca_cor', 'grau_instr', 'occ4',  'earnings_by_wid_cumul_norm1999', 'iota_level_0', 'gamma_level_0', 'cnae4', 'isic3', 'cnae'] + [i for i in analysis_data.columns if i.startswith('X_')]  + [i for i in analysis_data.columns if i.startswith('I_')]


analysis_data.loc[(analysis_data.idade>=25) & (analysis_data.idade<=55) & (analysis_data.earnings_by_wid_cumul_norm1999!=np.inf) & (analysis_data.earnings_by_wid_cumul_norm1999!=np.nan)]
