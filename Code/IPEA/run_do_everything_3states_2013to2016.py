...
if run_pull==True:
    ...
    for year in range(firstyear,lastyear+1):
        pull_one_year(year, 'cbo2002', savefile=root + '../Data/derived/raw_data_sbm_' + modelname + '_' + str(year) + '.p',state_codes=state_codes, age_lower=25, age_upper=55, othervars=['data_adm','data_deslig','data_nasc','tipo_salario','rem_dez_r','horas_contr','clas_cnae20'], parse_dates=['data_adm','data_deslig','data_nasc'], nrows=maxrows)
...
if run_append==True:
    for year in range(firstyear,lastyear+1):
        df = pickle.load( open(root + '../Data/derived/raw_data_sbm_' + modelname + '_' + str(year) + '.p', "rb" ) )
        if year>firstyear:
            appended = df.append(appended, sort=True)
        else:
            appended = df
        del df
    appended.to_pickle(root + '../Data/derived/appended_sbm_' + modelname + '.p')

################################################################################################
# Run SBM

appended = pd.read_pickle(root + '../Data/derived/appended_sbm_' + modelname + '.p')
occvar = 'cbo2002'
if run_sbm==True:
    ...
    pickle.dump( bipartite_edgelist,  open(root + '../Data/derived/bipartite_edgelist_'+modelname+'.p', "wb" ) )
    model = bisbm.bisbm()                                                                       
    model.create_graph(filename=root + '../Data/derived/bipartite_edgelist_'+modelname+'.p',min_workers_per_job=5)
    model.fit(n_init=1)
    ...
    pickle.dump( model, open(root + '../Data/derived/sbm_output/model_'+modelname+'.p', "wb" ), protocol=4 )
...
appended = pd.read_pickle(root + '../Data/derived/appended_sbm_3states_2013to2016.p')
