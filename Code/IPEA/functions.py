...
def create_earnings_panel(modelname, appended, firstyear_panel, lastyear_panel, sbm_modelname=None):
    ...
    model = pickle.load( open('../data/model_'+sbm_modelname+'.p', "rb" ) )        
    job_blocks    = pd.read_csv('../data/model_'+sbm_modelname+'_jblocks.csv')
    worker_blocks = pd.read_csv('../data/model_'+sbm_modelname+'_wblocks.csv', dtype={'wid':str})
    ...
    for l in range(model.L):
        ...
        gname = 'gamma_level_' + str(l)
        iname = 'iota_level_' + str(l)
        balanced.rename(columns={iname:'iota',gname:'gamma'}).to_csv('../export/panel_'+modelname+'_level_'+str(l)+'.csv.gz', index=False, compression='gzip', columns = ['wid_masked', 'jid_masked', 'year', 'cbo2002', 'cbo2002_first', 'clas_cnae20', 'clas_cnae20_first', 'sector_IBGE', 'c', 'real_hrly_wage_dec', 'ln_real_hrly_wage_dec', 'yob', 'iota', 'gamma'])
