...
def pull_one_year(year, occvar, savefile=None, municipality_codes=None, state_codes=None, nrows=None, othervars=None):
    print(year)
    now = datetime.now()
    ...
    if state_codes is not None:
        raw_data = raw_data[raw_data['uf'].isin(state_codes)] 
...
d = {}
for var in ['isic3','gamma_level_0','occ4Xmicro','iota_level_0','occ4']:
    print(var)
    # Earnings
...
# Create XD_gamma and IS_gamma
RHS_vals = {}
for group in ['gamma_level_0','iota_level_0','occ4','occ4Xmicro']:
    for e in ['earnings','emp']:
        temp = d[e+'_by_'+group+'_isic3'].merge(d[e+'_by_isic3'], on='isic3').merge(d[e+'_by_'+group], on=group).merge(cnae_trade_short.drop(columns='cnae').drop_duplicates(), on='isic3')
        for trade in ['X', 'I']:
            for hat in ['_hat','']:
                temp[trade+hat+'_'+group+'_isic3'+'_'+e] = (temp['total_'+e+'_'+group+'_isic3']/temp['total_'+e+'_'+group]) * (temp['Delta_'+trade+hat+'_isic3'] / temp['total_'+e+'_isic3'])
                temp[trade+hat+'_'+group+'_'+e] = temp.groupby(group)[trade+hat+'_'+group+'_isic3'+'_'+e].transform('sum')
        RHS_vals[group+'_'+e]=temp
        analysis_data = analysis_data.merge(temp[['isic3', group, 'X_hat_'+group+'_isic3_'+e, 'X_hat_'+group+'_'+e, 'X_'+group+'_isic3_'+e,'X_'+group+'_'+e, 'I_hat_'+group+'_isic3_'+e, 'I_hat_'+group+'_'+e, 'I_'+group+'_isic3_'+e, 'I_'+group+'_'+e]], on=['isic3',group],how='left',validate='m:1')
...
analysis_data = analysis_data.merge(temp_isic3[['isic3', 'X_hat_isic3_emp', 'X_isic3_emp', 'I_hat_isic3_emp', 'I_isic3_emp']], on='isic3',how='left',validate='m:1')
        
'''
# Should probably be deleted. Superceded by the XD_gamma stuff above.
...
# New attempt 9/19
# Goal: produce XD_gamma and IS_gamma
#analysis_data['Delta_X_gamma'] = (analysis_data.total_emp_gamma_level_0_isic3 / analysis_data.total_emp_gamma_level_0) * (analysis_data.Delta_X_isic3 / analysis_data.total_emp_isic3)
# Instead of merging everything back to analysis data, I think we should create a gamma-isic3 level data set and collapse by gamma to get XD_g and IS_g and then merge back on to analysis_data by gamma
...
# Subset for Stata
vars = ['codemun', 'idade', 'genero', 'raca_cor', 'grau_instr', 'occ4','occ4Xmicro',  'earnings_by_wid_cumul_norm1999', 'earnings_dec_2010usd_cumul', 'iota_level_0', 'gamma_level_0', 'cnae4', 'isic3', 'cnae'] + [i for i in analysis_data.columns if i.startswith('X_')]  + [i for i in analysis_data.columns if i.startswith('I_')]
