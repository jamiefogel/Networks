...
def create_earnings_panel_trade_shock(modelname, appended, firstyear_panel, lastyear_panel, sbm_modelname=None, sector_var='sector_IBGE'):
    ...
    #appended = appended.loc[appended['cbo1994'].str[0]!='0']
    ...
    appended.sort_values(by=['wid', 'year', 'rem_dez_sm'], inplace=True, ascending=True)
    appended['real_hrly_wage_dec'] = appended['rem_dez_sm']
    appended['ln_real_hrly_wage_dec'] = np.log(appended['rem_dez_sm'])
    ...
    balanced['occ2Xmeso'] = balanced.cbo1994.str[0:2] + '_' + balanced['code_meso'].astype('str')
    
    balanced['occ2'] = balanced.cbo1994.str[:2]
    balanced['occ4'] = balanced.cbo1994.str[:4]
    balanced['code_uf_cat'] = balanced['code_uf'].astype('category').cat.codes
    balanced[f'{sector_var}_state'] = np.where(balanced['code_uf'].isna()  | balanced[sector_var].isna(), np.nan, balanced[sector_var] + balanced['code_uf_cat'] * 15)
    #Identify first occupation/industry/occ2Xmeso worker is ever observed in
    print('Identifying workers first cbo1994 and clas_cnae20')
    occ_recode_cw = {}
    for var in ['cbo1994','occ2','occ4','occ2Xmeso', 'gamma_level_0'] :
    ...
    export_columns = ['wid_masked', 'jid_masked', 'year', sector_var, 'codemun', 'code_uf', 'code_meso', 'c', 'ln_real_hrly_wage_dec', 'real_hrly_wage_dec', 'iota', 'gamma', 'cbo1994', 'cbo1994_first', 'cbo1994_recode', 'cbo1994_first_recode',  'occ2', 'occ2_first', 'occ2_recode', 'occ2_first_recode', 'occ4', 'occ4_first', 'occ4_recode', 'occ4_first_recode', 'occ2Xmeso', 'occ2Xmeso_first', 'occ2Xmeso_recode', 'occ2Xmeso_first_recode', 'gamma_level_0_first','cnpj_raiz','fx_etaria','id_estab']
    for l in [0]: #range(model.L):
