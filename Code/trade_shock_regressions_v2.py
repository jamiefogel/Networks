...
## MERGING IOTAS AND GAMMAS -  I had to coerce wid to become numeric for this to work 
#import graph_tool as gt
modelname = 'trade_shock_mcmc'
...
gammas = pd.read_csv(root + 'Data/derived/sbm_output/model_' + modelname + '_jblocks.csv')[['jid', 'job_blocks_level_0']].rename(columns={'job_blocks_level_0':'gamma'})
iotas = pd.read_csv(root + 'Data/derived/sbm_output/model_' + modelname + '_wblocks.csv')[['wid', 'worker_blocks_level_0']].rename(columns={'worker_blocks_level_0':'iota'})


df = df.merge(gammas, on='jid', how='left', validate='m:1', indicator='_merge_gamma')

df = df.drop(['iota'], axis=1)
df['wid'] = pd.to_numeric(df['wid'], errors='coerce')  # 'coerce' will convert non-convertible values to NaN
df = df.merge(iotas, on='wid', how='left', validate='m:1', indicator='_merge_iota')

df['_merge_iota'].value_counts()
df['_merge_gamma'].value_counts()
...
df_filtered = df_filtered.merge(cbo1994_names, on='cbo1994', how='left', validate='m:1', indicator='_merge_cbo')
df_filtered['_merge_cbo'].value_counts()
...
iota_reg = save_essential_regression_results(iota_reg, filepath = root + 'Data/derived/trade_shock/iota_reg_' + modelname + '.pickle')
...
mmc_reg = save_essential_regression_results(mmc_reg, filepath = root + 'Data/derived/trade_shock/mmc_reg' + modelname + '.pickle')
...
occ4_reg = save_essential_regression_results(occ4_reg, filepath = root + 'Data/derived/trade_shock/occ4_reg' + modelname + '.pickle')
...
occ3_reg = save_essential_regression_results(occ3_reg, filepath = root + 'Data/derived/trade_shock/occ3_reg' + modelname + '.pickle')
...
occ2Xmeso_reg = save_essential_regression_results(occ2Xmeso_reg, filepath = root + 'Data/derived/trade_shock/occ2Xmeso_reg' + modelname + '.pickle')
...
###############################################################################################
# Merge on gammmas: NOW WE ARE MERGING THE NEW MCMC GAMMAS ABOVE
...
df_filtered.to_pickle( root + "/Data/derived/trade_shock/df_filtered" + modelname + ".p")
...
for k in ['mmc','iota','gamma','occ3','occ4', 'occ2Xmeso']:
    print(k,' unique values: ',df_filtered[k].value_counts().shape)
...
def get_nth_frequency(df_filtered, n, sizecutoff, varname):
    """
    Returns the frequency of 'cbo1994' and 'cbotrans' for the nth largest or smallest unique 'varname',
    ...
    n (int): Rank of the 'varname'. Positive for nth largest, negative for nth smallest.
    ...
    # Drop duplicates to ensure unique varname for each iota
    unique_temp = temp.drop_duplicates(subset=['iota', varname])

    # Ensure n is not zero
    if n == 0:
        raise ValueError("n cannot be zero.")

    # Step 2: Find the iota value with the nth largest or smallest unique varname depending on n
    if n > 0:
        if n > len(unique_temp):
            raise ValueError("n is greater than the number of available rows after filtering for largest.")
        nth_value_row = unique_temp.nlargest(n, varname).iloc[-1]
    else:
        if -n > len(unique_temp):
            raise ValueError("n is greater than the number of available rows after filtering for smallest.")
        nth_value_row = unique_temp.nsmallest(-n, varname).iloc[-1]

    # Print iota and its corresponding varname value
    print(f"iota: {nth_value_row['iota']}, {varname}: {nth_value_row[varname]}")
    #print(f"iota: {nth_value_row['iota']}, coef_iota: {nth_value_row['coef_iota']}")
    iota = nth_value_row['iota']
    varvalue = nth_value_row[varname]
    ...
    return frequency_cbo1994, iota, varvalue

varname =  'coef_iota'
for i in [-1,-2,1,2]:
    table, iota, varvalue = get_nth_frequency(df_filtered, i, 10000, varname)
    print(table.head(10))
    print(iota)
    print(varvalue)
...
#############################################################################################
###### Avg Predicted values per iota and occupation counts
#############################################################################################

df_filtered['iota_fitted_values_avg'] = df_filtered.groupby('iota')['iota_fitted_values'].transform('mean')

varname =  'iota_fitted_values_avg'
for i in [-1,-2,1,2]:
    table, iota, varvalue = get_nth_frequency(df_filtered, i, 10000, varname)
    print(table.head(10))
    print(iota)
