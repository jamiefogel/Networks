...
* This file generates regional raw average earnings for the years 1991, 1995,
* 2000, 2005 and 2010.
* Results are used in Table 1 -- Descriptive Statistics
...
log using ${result}RegionalAvgEarnings.log, replace
...
foreach year in 1991 1995 2000 2005{
	...
	* Number of observations per mmc
	...
	collapse (mean) rem_dez, by(mmc)
	
	gen year = `year'
	
	sort mmc
	
	save ${result}mmc_avg_earn_`year', replace
...
save ${result}mmc_avg_earn_2010, replace
...
use ${result}mmc_avg_earn_1991, clear


foreach year in 1995 2000 2005 2010{
	append using ${result}mmc_avg_earn_`year'
}

label var mmc           "Minimum Comparable Microregions"
label var year          "Year"
label var rem_dez       "Average Earnings in December"

sort mmc year

save ${result}mmcAvgEarnings_1991_2010, replace

foreach year in 1991 1995 2000 2005 2010{
	erase ${result}mmc_avg_earn_`year'.dta
}
